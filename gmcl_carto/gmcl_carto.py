...
bagname = "WithPeopleReality_withoutodom_grndtruth.bag"
class Handler:
    ...
    def activateCartographer(self, msg, period):
        # start another time
        subprocess.Popen('roslaunch cartographer_ros demo_my_robot_localization.launch load_state_filename:=/root/workspace/newdisk/WithoutPeopleEmptyNewStart.bag.pbstream bag_filename:=/root/workspace/newdisk/'+self.bagname + ' start_time:={}'.format(period-0.5), shell=True)
        while True:
           topics = rospy.get_published_topics()
           found = False
           for item in topics:
    ...
    def poseCallback(self, msg):
        ...
        if self._enabled == False: # check for global_localization enabled or not
            ...
            if cov[0] < 0.05 and cov[7] < 0.05 and cov[35] < 0.05: # already converge 
...
if __name__ == "__main__":       
    ...
    subprocess.Popen('rosbag record /clock /tf /tf_static /robot/robotnik_base_control/odom tracked_pose /gmcl_pose /gmcl_particlecloud /gmcl_SER -o '+ bagname[:-4], shell=True)
